...
export const RepoCard: React.FC<RepoCardProps> = ({ token, owner, repo, defaultBranch }) => {
  const { data: workflows, isLoading, error } = useRepoWorkflows(token, owner, repo, defaultBranch);

  if (isLoading) return <div className="p-4 bg-gray-800 rounded animate-pulse h-32"></div>;
  if (error) return <div className="p-4 bg-red-900/20 text-red-300 rounded border border-red-800">Error loading {repo}</div>;
  ...
  return (
    <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden flex flex-col">
      <div className="p-4 bg-gray-900/50 border-b border-gray-700 flex justify-between items-center">
        ...
        <span className="text-xs text-gray-500 bg-gray-800 px-2 py-1 rounded">{defaultBranch}</span>
      </div>
